  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex overflow-x-auto max-w-full whitespace-nowrap bg-bg-alt/50 p-1.5 rounded-lg gap-1 border border-border scrollbar-none">
        ...
        <button
          type="button"
          onClick={() => setActivoTab('planes')}
          className={`py-2 px-5.5 text-[0.88rem] font-bold rounded-md transition-all duration-200 cursor-pointer shrink-0 ${activoTab === 'planes' ? 'bg-white text-primary shadow-sm border border-border' : 'text-text-muted hover:text-primary hover:bg-white/50'}`}
        >
          📋 Planes de Estudio
        </button>
        <button
      ...
      <div className="mt-2">
        ...
        {activoTab === 'planes' && <PlanesEstudio />}
