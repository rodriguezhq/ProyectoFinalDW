...
export default function ModalDetalleActa({
    ...
    return (
        ...
        >
            <div 
                className="bg-white rounded-none border border-border shadow-2xl max-w-4xl w-full flex flex-col max-h-[90vh] overflow-hidden animate-scale-in"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="bg-slate-50 border-b border-border p-5 flex justify-between items-start rounded-none">
                    <div>
                ...
                <div className="p-5 overflow-y-auto flex-1 min-h-[300px]">
                    {/* Docente a cargo */}
                    <div className="mb-4 bg-slate-50 border border-border p-3 flex flex-col sm:flex-row justify-between sm:items-center gap-2 rounded-none">
                                            ...
                                            <tr key={alum.id_matricula_detalle} className={`border-b border-border/60 hover:bg-slate-50/50 ${isEven ? 'bg-white' : 'bg-slate-50/20'}`}>
                                                ...
                                                <td className="p-2 text-center">
                                                    {sinNota ? (
                                                        ...
                                                        <span className={`px-2 py-0.5 font-extrabold uppercase text-[9px] rounded-none ${
                                                            isAprobado 
                                                                ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' 
                ...
                {/* Footer */}
                <div className="bg-slate-50 border-t border-border p-5 flex justify-end gap-3 rounded-none">
                    <button
                        type="button"
                        onClick={() => setModalActa(null)}
